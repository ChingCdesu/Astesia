...
**Scope:** every Astesia GPUI surface and reusable visual primitive.  
**Rule:** use the pinned Zed UI component, semantic theme roles, dynamic spacing, text sizes, and
elevation APIs as the default design system. Astesia adds product composition and engine identity;
it does not fork Zed's component styling into fixed local values.  
**Why:** copying Zed's current pixels loses theme, UI scale, density, state, and accessibility
behavior and drifts when the pinned revision changes.  
**Exceptions:** a product-specific control with no suitable Zed primitive may compose raw GPUI from
Zed theme and spacing APIs; record the missing primitive and verify every state.  
**Source:** user decision; `Cargo.toml` pinned Zed revision; `references/zed-design.md`.  
**Bad:** hard-code a button's background, radius, height, and focus border to resemble Zed.  
**Good:** use `Button`, `ButtonStyle`, `ButtonSize`, `ElevationIndex`, and semantic theme roles.
...
**Source:** `DESIGN.md`, The Identity-Is-Not-Action Rule; `references/zed-design.md`.  
