...
impl DataGridItem {
    ...
    pub(super) fn export_clickhouse_csv(
        &mut self,
        _: &ClickEvent,
        window: &mut Window,
        cx: &mut Context<Self>,
        ...
        let default_name = format!("{}.csv", self.state.table().name());
        ...
        cx.spawn_in(window, async move |item, cx| {
            ...
            item.update_in(cx, |item, window, cx| match response {
                Ok(Ok(Some(path))) => item.start_csv_export(path, window, cx),
                Ok(Ok(None)) => {}
                Ok(Err(error)) => {
        ...
        cx.spawn(async move |item, cx| {
            ...
            item.update(cx, |item, cx| {
                ...
                item.operation_notice = Some(match result {
                    Ok((count, path)) => GridNotice::Success(format!(
                        "{} {count} {} {path}",
                        text(item.settings.read(cx).language(), "已导出", "Exported"),
                        text(item.settings.read(cx).language(), "行到", "row(s) to")
                    )),
