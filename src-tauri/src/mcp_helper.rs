...
impl McpHelperState {
    ...
    pub async fn start(&self, port: u16, auth_token: String) -> Result<McpServiceStatus, String> {
        ...
        let command = match self.app_handle.shell().sidecar(SIDECAR_NAME) {
            ...
            Err(error) => {
                ...
                if self.record_start_error(generation, message.clone()).await {
                    return Err(message);
                }
        ...
        let (receiver, child) = match command
            ...
            .env("ASTESIA_MCP_AUTH_TOKEN", &config.auth_token)
            .spawn()
        {
            Ok(process) => process,
            Err(error) => {
    ...
    pub async fn stop(&self) -> Result<McpServiceStatus, String> {
        let (child, generation) = {
            let mut inner = self.inner.lock().await;
            match inner.phase {
                McpServicePhase::Stopped => return Ok(self.snapshot(&inner)),
                McpServicePhase::Stopping => (None, inner.generation),
        ...
        let message = kill_error.unwrap_or_else(|| {
    ...
    async fn record_start_error(&self, generation: u64, error: String) -> bool {
        let mut inner = self.inner.lock().await;
        ...
        while let Some(event) = receiver.recv().await {
            match event {
                ...
                CommandEvent::Terminated(payload) => {
                    ...
                    record_termination(&inner, generation, message).await;
                    return;
                }
                _ => {}
            }
...
async fn record_termination(inner: &Arc<Mutex<RuntimeState>>, generation: u64, error: String) {
