...
| D08 | Save a batch of staged grid changes | MySQL, PostgreSQL, SQLite, SQL Server | M5 | Updates, inserts, and deletes target the original row identity in one transaction; failures roll back the whole batch and keep all staged changes editable. |
